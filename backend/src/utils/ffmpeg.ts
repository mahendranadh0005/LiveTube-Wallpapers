...
export function processVideo(options: ProcessOptions, onProgress?: (percent: number) => void): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    if (options.effects.includes('blur_background')) {
      ...
      if (options.aspectRatio === '9:16') numericalRatio = 9 / 16;
      if (options.aspectRatio === '21:9') numericalRatio = 21 / 9;
    ...
    if (options.burnSubtitles && options.subtitlePath) {
      ...
      const relativeSubPath = path.relative(process.cwd(), options.subtitlePath).replace(/\\/g, '/');
